...
through E4M9. This identifies the bundled resource as full Ultimate DOOM data;
...
### Earlier control availability fix

The browser platform previously hardcoded `isTouchPlatform` to `false`, so the
game could render on a phone without exposing any controls. It now reads
`navigator.maxTouchPoints` when the property is requested. A browser reporting
touch support gets the existing shared controls; mouse-only browsers retain
keyboard input. Capability detection also covers tablets using desktop browser
identification and computers with both mouse and touch input.
...
touch points and transitions from zero to five and back. Tests restore the
original browser property descriptor in `finally`. These checks cover control
availability and avoid caching an initial mouse-only result; they do not exercise
physical touch hardware or simultaneous multitouch input. Mobile browser
emulation and physical-device verification remain distinct from these tests.

That production build was checked using Edge's iPhone SE device emulation in
portrait and landscape. Controls appeared in both orientations; ESC and FIRE
opened the menu and started E1M1, and right-side dragging changed the view.
The landscape launcher scrolled to the game-file picker. Sustained joystick
movement and simultaneous movement/fire were not established by the brief
automated gestures and still require a physical-device check.

### Movement, turning and fire with two thumbs

The availability fix exposed a control layout that still mapped the left stick's
horizontal axis to strafe. Moving, turning and firing therefore required a
separate right-side turning gesture while reaching for FIRE. The stick now maps
its horizontal axis to the engine's left/right turn keys and its vertical axis
to forward/back. Diagonal input combines movement and turning; the right thumb
can hold FIRE continuously. Right-side dragging remains optional for fine turns.

`TouchStickController` owns this mapping with a normalized dead zone. The shared
stick responds on touch down without waiting for a drag threshold, displays a
moving knob, and held buttons display their pressed state. Each control tracks
the pointer that started it until that pointer lifts or is cancelled, including
when it moves outside the control circle. Other fingers do not replace that
pointer. Releasing or cancelling a gesture releases that control's keys;
disposing the controls releases all held touch keys. Releasing FIRE preserves
stick movement and turning, and releasing the stick preserves FIRE.

New common regression cases check simultaneous forward/turn/fire, independent
stick and FIRE release, direction reversal, the inclusive dead-zone boundary,
duplicate press prevention, and disposal cleanup. These cases describe controller
behavior. Eight additional cases run the actual gesture handler against queued
Compose pointer events, covering immediate press, capture outside bounds,
independent contacts, release/re-press, ownership and cancellation. A real-engine
regression checks that forward, turn and fire coexist without side movement,
and that releases remain independent.

Validation passes 188 JVM tests and 165 browser tests across the application and
engine, the production Wasm build, and architecture/source-layout checks for 423
Kotlin files. The earlier browser emulation results above do not verify the
changed gesture handling or simultaneous physical multitouch. Physical-device
