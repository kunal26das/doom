...
On phones/tablets, including mobile browsers: left virtual stick (move + strafe),
drag the right half to turn, and use the on-screen FIRE / USE / weapon-cycle /
RUN / MAP / ESC buttons. Touch-capable browsers show these controls automatically,
including tablets using a desktop browser identity. Tap ESC to open the menu,
move the stick up/down to select an item, and tap FIRE to confirm. Browser page
gestures are disabled so dragging controls stays with the game.
