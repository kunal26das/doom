...
and choose **New Game** to select an episode and difficulty.

If you previously imported another game, the app restores and starts that file
instead. If there is no remembered file, or it cannot be restored, the bundled
game starts automatically without a file-selection screen.

To use another local copy, choose **CHOOSE GAME** after quitting or an engine
error. In the browser, drag `DOOM.WAD` from your installation onto that screen;
the drop area highlights while a file is over the page. You can also select
**LOAD DOOM.WAD** and choose the file; the picker is available on every platform.
`DOOM2.WAD` is also supported. Import one `.wad` file at a time, up to 64 MB.
The app remembers an imported file on this device for later visits. Choosing
**PLAY DOOM** on that screen starts the bundled game without replacing a
remembered import.
...
same game data; **CHOOSE GAME** opens the game-file selection screen. Saves and
...
| Game data | The current `doom1.wad` resource contains all four Ultimate DOOM episodes and starts automatically by default; local IWAD import remains available. |
...
**CHOOSE GAME** opens the game-file selection screen. Regression coverage and remaining
