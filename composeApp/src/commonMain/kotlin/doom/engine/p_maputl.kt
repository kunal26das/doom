...
/** C union { mobj_t* thing; line_t* line; } d -- flattened into two refs. */
class intercept_t {
    var frac: fixed_t = 0   // along trace line
    var isaline = false
...
typealias traverser_t = (intercept_t) -> Boolean

const val PT_ADDLINES = 1
const val PT_ADDTHINGS = 2
const val PT_EARLYOUT = 4

//
// P_AproxDistance
// Gives an estimation of distance (not exact)
//
fun P_AproxDistance(dx: fixed_t, dy: fixed_t): fixed_t {
    val dx = abs(dx)
    val dy = abs(dy)
    if (dx < dy)
...
// Returns 0 or 1
//
fun P_PointOnLineSide(x: fixed_t, y: fixed_t, line: line_t): Int {
    if (line.dx == 0) {
        if (x <= line.v1.x)
...
//
fun P_BoxOnLineSide(tmbox: IntArray, ld: line_t): Int {
    var p1 = 0
    var p2 = 0
...
// Returns 0 or 1.
//
fun P_PointOnDivlineSide(x: fixed_t, y: fixed_t, line: divline_t): Int {
    if (line.dx == 0) {
        if (x <= line.x)
            return if (line.dy > 0) 1 else 0
...
// P_MakeDivline
//
fun P_MakeDivline(li: line_t, dl: divline_t) {
    dl.x = li.v1.x
    dl.y = li.v1.y
    dl.dx = li.dx
    dl.dy = li.dy
...
fun P_LineOpening(linedef: line_t) {
...
//
fun P_UnsetThingPosition(thing: mobj_t) {
...
//
fun P_SetThingPosition(thing: mobj_t) {
    // link into subsector
    val ss = R_PointInSubsector(thing.x, thing.y)
    thing.subsector = ss
...
// to it.
//
fun P_BlockLinesIterator(x: Int, y: Int, func: (line_t) -> Boolean): Boolean {
    if (x < 0
        || y < 0
        || x >= bmapwidth
        || y >= bmapheight
...
//
fun P_BlockThingsIterator(x: Int, y: Int, func: (mobj_t) -> Boolean): Boolean {
    if (x < 0
        || y < 0
        || x >= bmapwidth
        || y >= bmapheight
...
// INTERCEPT ROUTINES
//
val intercepts = Array(MAXINTERCEPTS) { intercept_t() }
var intercept_p = 0    // index into intercepts (C: intercept_t*)
...
//
fun PIT_AddLineIntercepts(ld: line_t): Boolean {
    val s1: Int
    val s2: Int
...
//
fun PIT_AddThingIntercepts(thing: mobj_t): Boolean {
    val x1: fixed_t
    val y1: fixed_t
    val x2: fixed_t
    val y2: fixed_t
...
// for all lines.
//
fun P_TraverseIntercepts(func: traverser_t, maxfrac: fixed_t): Boolean {
