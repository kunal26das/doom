...
fun P_SetMobjState(mobj: mobj_t, state: Int): Boolean {
    var state = state
    var st: state_t
...
// P_XYMovement
//
const val STOPSPEED = 0x1000
const val FRICTION = 0xe800
...
// P_NightmareRespawn
//
fun P_NightmareRespawn(mobj: mobj_t) {
    val x: fixed_t
    val y: fixed_t
    val z: fixed_t
    val ss: subsector_t
...
// P_SpawnMobj
//
fun P_SpawnMobj(x: fixed_t, y: fixed_t, z: fixed_t, type: Int): mobj_t {
    val mobj: mobj_t
    val st: state_t
    val info: mobjinfo_t
...
// P_RemoveMobj
//
val itemrespawnque = Array(ITEMQUESIZE) { mapthing_t() }
val itemrespawntime = IntArray(ITEMQUESIZE)
var iquehead = 0
var iquetail = 0

fun P_RemoveMobj(mobj: mobj_t) {
...
//  between levels.
//
fun P_SpawnPlayer(mthing: mapthing_t) {
    val p: player_t
    val x: fixed_t
    val y: fixed_t
    val z: fixed_t
...
// P_SpawnBlood
//
fun P_SpawnBlood(x: fixed_t, y: fixed_t, z: fixed_t, damage: Int) {
    var z = z
    val th: mobj_t
...
//
fun P_CheckMissileSpawn(th: mobj_t) {
    th.tics -= P_Random() and 3
    if (th.tics < 1)
        th.tics = 1
...
// P_SpawnMissile
//
fun P_SpawnMissile(source: mobj_t, dest: mobj_t, type: Int): mobj_t {
    val th: mobj_t
    var an: angle_t
    var dist: Int
...
//
fun P_SpawnPlayerMissile(source: mobj_t, type: Int) {
    val th: mobj_t
    var an: angle_t
