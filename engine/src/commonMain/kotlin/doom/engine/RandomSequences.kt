...
private val randomTable = intArrayOf(
...
/** Owns both independent random cursors; callers cannot mutate either cursor or the LUT. */
internal class RandomSequences {
    var gameplayIndex: Int = 0
        private set
    var presentationIndex: Int = 0
        private set

    fun nextGameplay(): Int {
        gameplayIndex = (gameplayIndex + 1) and 0xff
        return randomTable[gameplayIndex]
    }
    fun nextPresentation(): Int {
        presentationIndex = (presentationIndex + 1) and 0xff
        return randomTable[presentationIndex]
    }
    fun reset() { gameplayIndex = 0; presentationIndex = 0 }
}
