...
internal var DoomEngineCore.sightzstart: fixed_t
    get() = statePSight.sightzstart
    set(value) { statePSight.sightzstart = value }
internal var DoomEngineCore.topslope: fixed_t
    get() = statePSight.topslope
    set(value) { statePSight.topslope = value }
internal var DoomEngineCore.bottomslope: fixed_t
...
internal var DoomEngineCore.t2x: fixed_t
    get() = statePSight.t2x
    set(value) { statePSight.t2x = value }
internal var DoomEngineCore.t2y: fixed_t
    get() = statePSight.t2y
...
//
internal fun DoomEngineCore.P_DivlineSide(x: fixed_t, y: fixed_t, node: divline_t): Int {
    if (node.dx == 0) {
        if (x == node.x)
            return 2
...
internal fun DoomEngineCore.P_DivlineSide(x: fixed_t, y: fixed_t, node: node_t): Int {
    if (node.dx == 0) {
        if (x == node.x)
            return 2
...
//
internal fun DoomEngineCore.P_InterceptVector2(v2: divline_t, v1: divline_t): fixed_t {
...
//
internal fun DoomEngineCore.P_CrossSubsector(num: Int): Boolean {
    // #ifdef RANGECHECK
...
//
internal fun DoomEngineCore.P_CrossBSPNode(bspnum: Int): Boolean {
...
// Uses REJECT.
//
internal fun DoomEngineCore.P_CheckSight(t1: mobj_t, t2: mobj_t): Boolean {
