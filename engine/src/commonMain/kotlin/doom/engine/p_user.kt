...
//
internal fun DoomEngineCore.P_Thrust(player: player_t, angle: angle_t, move: fixed_t) {
...
//
internal fun DoomEngineCore.P_CalcHeight(player: player_t) {
    val angle: Int
    val bob: fixed_t
...
// P_MovePlayer
//
internal fun DoomEngineCore.P_MovePlayer(player: player_t) {
    val cmd: ticcmd_t
...
internal val DoomEngineCore.ANG5: angle_t
    get() = statePUser.ANG5

internal fun DoomEngineCore.P_DeathThink(player: player_t) {
    ...
    if (player.attacker != null && player.attacker !== player.mo) {
        val angle: angle_t = FixedGeometry.angleBetween(player.mo!!.x,
            player.mo!!.y,
            player.attacker!!.x,
            player.attacker!!.y)
...
// P_PlayerThink
//
internal fun DoomEngineCore.P_PlayerThink(player: player_t) {
    val cmd: ticcmd_t
