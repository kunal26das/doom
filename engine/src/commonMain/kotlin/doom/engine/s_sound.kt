...
internal val DoomEngineCore.snd_prefixen
    get() = stateSoundPlayback.snd_prefixen

internal const val S_MAX_VOLUME = 127

// when to clip out sounds
// Does not fit the large outdoor areas.
internal const val S_CLIPPING_DIST: fixed_t = 1200 * 0x10000

// Distance tp origin when sounds should be maxed out.
// This should relate to movement clipping resolution
// (see BLOCKMAP handling).
// Originally: (200*0x10000).
internal const val S_CLOSE_DIST: fixed_t = 160 * 0x10000
...
internal const val S_STEREO_SWING: fixed_t = 96 * 0x10000
...
private var DoomEngineCore.channels: Array<channel_t>
    get() = stateSoundPlayback.channels
    set(value) { stateSoundPlayback.channels = value }

internal var DoomEngineCore.snd_SfxVolume
    get() = stateSoundPlayback.snd_SfxVolume
    set(value) { stateSoundPlayback.snd_SfxVolume = value }

internal var DoomEngineCore.snd_MusicVolume
    get() = stateSoundPlayback.snd_MusicVolume
    set(value) { stateSoundPlayback.snd_MusicVolume = value }


private var DoomEngineCore.mus_paused
    get() = stateSoundPlayback.mus_paused
    set(value) { stateSoundPlayback.mus_paused = value }

private var DoomEngineCore.mus_playing: musicinfo_t?
    get() = stateSoundPlayback.mus_playing
    set(value) { stateSoundPlayback.mus_playing = value }
...
//
internal fun DoomEngineCore.S_Init(sfxVolume: Int, musicVolume: Int) {
    var i: Int
...
//
internal fun DoomEngineCore.S_Start() {
    var cnum: Int
    val mnum: Int
...
internal fun DoomEngineCore.S_StartSoundAtVolume(origin_p: soundorigin_t?, sfx_id: Int, volume: Int) {
    var volume = volume
    val rc: Int
    var sep: Int
    var pitch: Int
...
internal fun DoomEngineCore.S_StartSound(origin: soundorigin_t?, sfx_id: Int) {
...
internal fun DoomEngineCore.S_StopSound(origin: soundorigin_t?) {
    var cnum: Int
...
//
internal fun DoomEngineCore.S_PauseSound() {
...
internal fun DoomEngineCore.S_ResumeSound() {
    if (mus_playing != null && mus_paused) {
...
//
internal fun DoomEngineCore.S_UpdateSounds(listener_p: mobj_t?) {
    var audible: Int
    var cnum: Int
    var volume: Int
    var sep: Int
...
internal fun DoomEngineCore.S_SetMusicVolume(volume: Int) {
...
internal fun DoomEngineCore.S_SetSfxVolume(volume: Int) {
    if (volume < 0 || volume > 127)
...
//
internal fun DoomEngineCore.S_StartMusic(m_id: Int) {
    S_ChangeMusic(m_id, 0)
}

internal fun DoomEngineCore.S_ChangeMusic(musicnum: Int, looping: Int) {
...
internal fun DoomEngineCore.S_StopMusic() {
    if (mus_playing != null) {
        if (mus_paused)
            I_ResumeSong(mus_playing!!.handle)
...
internal fun DoomEngineCore.S_StopChannel(cnum: Int) {
    var i: Int
    val c = channels[cnum]
...
//
internal fun DoomEngineCore.S_AdjustSoundParams(listener: mobj_t, source: soundorigin_t, vsp: IntArray): Int {
...
//
internal fun DoomEngineCore.S_getChannel(origin: soundorigin_t?, sfxinfo: sfxinfo_t): Int {
...
internal fun DoomEngineCore.S_HostMusicId(): Int? {
    val music = mus_playing ?: return null
    return S_music.indexOf(music).takeIf { it > mus_None }
}

/** Clear driver-owned handles even if one host cleanup operation fails. */
internal fun DoomEngineCore.S_DetachHostAudio() {
    ...
    for (channel in channels) {
        if (channel.sfxinfo != null) attempt { I_StopSound(channel.handle) }
        channel.sfxinfo = null
        channel.origin = null
    }
    mus_playing?.let { music ->
        attempt { I_StopSong(music.handle) }
        attempt { I_UnRegisterSong(music.handle) }
        music.data = null
        music.handle = 0
    }
    mus_playing = null
