...
class ScreenWipeTest {
    ...
    fun colorTransformUsesUnsignedValuesClampsAtTheTargetAndCompletesOnTheFollowingTick() {
        ...
        assertFalse(wipe.advance(WIPE_COLOR_X_FORM, 2))
        ...
        assertTrue(wipe.advance(WIPE_COLOR_X_FORM, 2))
    }
    ...
    fun meltPreservesPixelPairsColumnDelaysAndVanillaCompletionTiming() {
        ...
        assertFalse(wipe.advance(WIPE_MELT, 1))
        ...
        assertTrue(wipe.advance(WIPE_MELT, 1))
    ...
    fun meltClampsMinusSixteenColumnDelayToMinusFifteen() {
        ...
        capture(wipe, buffers, start, end)
        assertFalse(wipe.advance(WIPE_MELT, 15))
    ...
    fun batchedMeltTicksMatchIndividualTicksWithoutSharingAnotherTransitionsState() {
        ...
        repeat(4) { assertFalse(first.advance(WIPE_MELT, 1)) }
        assertContentEquals(start, secondBuffers[0])
        assertFalse(second.advance(WIPE_MELT, 4))
        assertContentEquals(firstBuffers[0], secondBuffers[0])
        assertTrue(first.advance(WIPE_MELT, 1))
        assertTrue(second.advance(WIPE_MELT, 1))
    }
    ...
    fun aCompletedTransitionCanCaptureAndWipeTheNextPairOfScreens() {
        ...
        assertFalse(wipe.advance(WIPE_COLOR_X_FORM, 10))
        assertTrue(wipe.advance(WIPE_COLOR_X_FORM, 10))
    ...
    @Test
    fun nonpositiveTicksLeaveNewAndActiveTransitionsUnchanged() {
        for (kind in intArrayOf(WIPE_COLOR_X_FORM, WIPE_MELT)) {
            val buffers = FrameBuffers(width = 4, height = 4)
            var randomCalls = 0
            val wipe = ScreenWipe(buffers) { randomCalls++; 0 }
            capture(wipe, buffers, start, end)

            for (ticks in intArrayOf(0, -1, Int.MIN_VALUE)) {
                assertFalse(wipe.advance(kind, ticks))
                assertContentEquals(start, buffers[0])
            }
            assertEquals(0, randomCalls, "A clock stall must not initialize the melt")

            assertFalse(wipe.advance(kind, 1))
            val progressed = buffers[0].copyOf()
            val callsAfterFirstTick = randomCalls
            for (ticks in intArrayOf(0, -1, Int.MIN_VALUE)) {
                assertFalse(wipe.advance(kind, ticks))
                assertContentEquals(progressed, buffers[0])
                assertEquals(callsAfterFirstTick, randomCalls)
            }
        }
    }

    @Test
    fun maximumClockGapFinishesPixelsWithoutReplayingEmptyMeltTicksOrOverflowingColors() {
        for (kind in intArrayOf(WIPE_COLOR_X_FORM, WIPE_MELT)) {
            val buffers = FrameBuffers(width = 4, height = 4)
            var randomCalls = 0
            val wipe = ScreenWipe(buffers) { randomCalls++; 15 }
            capture(wipe, buffers, start, end)

            assertFalse(wipe.advance(kind, Int.MAX_VALUE))
            assertContentEquals(end, buffers[0])
            val callsAfterProgress = randomCalls
            assertFalse(wipe.advance(kind, 0))
            assertFalse(wipe.advance(kind, -1))
            assertContentEquals(end, buffers[0])
            assertTrue(wipe.advance(kind, 1), "Keep vanilla's next-tick completion timing")
            assertEquals(callsAfterProgress, randomCalls, "A clock stall must not restart the transition")
        }
    }
