...
class InstanceIsolationTest {
    ...
    fun mutableDefinitionsConfigInputAndRandomStreamsBelongToTheirEngine() {
        ...
        assertNotSame(first.S_sfx[1], second.S_sfx[1])
        ...
        first.S_sfx[1].lumpnum = 45
        first.chat_macros[0] = "first engine"
        first.P_Random()
        first.D_PostEvent(event_t(ev_keydown, 'w'.code))
        assertEquals(originalTics, second.states[0].tics)
        assertEquals(0x19, second.forwardmove[0])
        assertEquals(-1, second.S_sfx[1].lumpnum)
        assertEquals(0, second.prndindex)
        assertEquals(0, second.inputQueue.pendingCount)
        assertTrue(second.chat_macros[0] != "first engine")
    }
    ...
    fun failedPersistenceStillDropsHostAndResumesMusicWithNewDriver() {
        ...
        try {
            ...
            assertNull(engine.core.S_HostMusicId())
            engine.detach() // repeated detachment must preserve the pending song
            ticks = 1000
            ...
            assertEquals(0, engine.core.I_GetTime())
