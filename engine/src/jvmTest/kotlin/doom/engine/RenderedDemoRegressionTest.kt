...
class RenderedDemoRegressionTest {
    // Captured with the pre-refactor engine binary and the bundled Ultimate DOOM
    // v1.9 WAD. These protect this port's output, not parity with the DOS renderer.
    // The digest includes the first five frames, then frames at gametic % 35 == 0.
    @Test
    ...
    private fun verify(mode: String, demo: String, tics: Int, frames: Int, hash: String) {
        ...
        val process = ProcessBuilder(
            java.absolutePath, "-cp", classpath, "doom.engine.RenderedDemoVerificationProcess", wad, demo, mode,
